export class iFunction {
  ...
  getSystemDateByName(strPhanHe: string): any {
    const jsonArray = this.getSystemDate();
    if (jsonArray && jsonArray.length > 0) {
      const rt = {THANG: 1, NAM: 2017};
  ...
  baseCalendar_Blur(event: any) {
    const val = event.path[0].value.split('/');
    if (val.length == 2) {
      this.dtThangNam = new Date(val[1] as number, (val[0] as number) - 1, this.dtThangNam ? this.dtThangNam.getDate() : 1);
    }
    else if (val.length == 3) {
      this.dtThangNam = new Date(val[2] as number, (val[1] as number) - 1, val[0] as number);
 }
  ...
  getUserInformation(): any {
    try {
      const strResult = this.getCookie('USER_INFO');
      if (strResult) {
  ...
  getSystemDate(): any[] {
    try {
      const strResult = this.getCookie('THANG_NAM_HT');
      if (strResult) {
        // const json = JSON.parse(strResult);
      }
      // return json;
      ...
      console.log(e);
      const strReturn = [{MA_DVIQLY: 'PC06AA', PHAN_HE: 'CD', THANG: 6, NAM: 2017}];
      return strReturn;
    }
  }
  ...
  getSystemParam(): any[] {
    try {
      const strResult = this.getCookie('THAM_SO_HT');
      if (strResult) {
        // const json = JSON.parse(strResult);
      }
      // return json;
  ...
  getMenuInfo(): any[] {
    try {
      const strResult = this.getCookie('MENU_INFO');
      if (strResult) {
        // const json = JSON.parse(strResult);
      }
      // return json;
  ...
  ConvertCurrencyToEnglish(inumber: string): string {
    ...
    if (DecimalPlace > 0) {
      const thapphan = inumber.substring(DecimalPlace + 1) + '00';
  ...
  ConvertTens(MyTens: string) {
    ...
    if (Number(this.Left(MyTens, 1)) == 1) {
      ...
      const tempVal = Number(this.Left(MyTens, 1));
  ...
  RemoveMark(s: string) {
    ...
    let chnew = '';
    if (s.length != 0) {
      const char1 = 'â, ă, ấ, ầ, ậ, ẫ, ẩ, ắ, ằ, ẵ, ẳ, ặ, á, à, ả, ã, ạ';
      const char2 = 'Â, Ă, Ấ, Ầ, Ậ, Ẫ, Ẩ, Ắ, Ằ, Ẵ, Ẳ, Ặ, Á, À, Ả, Ã, Ạ';
      const char3 = 'ó, ò, ỏ, õ, ọ, ô, ố, ồ, ổ, ỗ, ộ, ơ, ớ, ờ, ợ, ở, ỡ';
      const char4 = 'Ó, Ò, Ỏ, Õ, Ọ, Ô, Ố, Ồ, Ổ, Ỗ, Ộ, Ơ, Ớ, Ờ, Ợ, Ở, Ỡ';
      const char5 = 'ư, ứ, ừ, ự, ử, ữ, ù, ú, ủ, ũ, ụ';
      const char6 = 'Ư, Ứ, Ừ, Ự, Ử, Ữ, Ù, Ú, Ủ, Ũ, Ụ';
      const char7 = 'ê, ế, ề, ệ, ể, ễ, è, é, ẻ, ẽ, ẹ';
      const char8 = 'Ê, Ế, Ề, Ệ, Ể, Ễ, È, É, Ẻ, Ẽ, Ẹ';
      const char9 = 'í, ì, ị, ỉ, ĩ';
      const char10 = 'Í, Ì, Ỉ, Ĩ, Ị';
      const char11 = 'ý, ỳ, ỵ, ỷ, ỹ';
      const char12 = 'Ý, Ỳ, Ỵ, Ỷ, Ỹ';
      const char13 = 'đ';
      const char14 = 'Đ';
      const Char: string[] = [];
      Char.push(char1);
